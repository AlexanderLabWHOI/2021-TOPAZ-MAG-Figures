...
```{r Pheatmap, fig.height=16, fig.width=15}
# pdf("../si-figures/SI-SAR-Dictyocho-MAG-MAG-relabun-bysample.pdf", h = 16, w = 15)
pheatmap(pheat_mags,
...
# pdf("../si-figures/SI-stramenopile-MAG-map.pdf", w=17,h=20)
map_world() +
...
trophy <- read.csv("../data/trophy_data.csv")
# head(mag_rename_key)
# head(trophy)
...
# pdf("../si-figures/SI-SAR-Dictyocho-MAG-KEGG-intersection.pdf", w = 15, h = 15)
metaT_mapped_trophy %>% 
  ...
  labs(x = "Frequency across MAGs",
       ...
       title = "Metatranscriptome reads that mapped to dictyochophyte MAGs")
# dev.off()
```
...
classify_keggs <- metaT_mapped %>% 
  ...
  add_column(value = 1) %>% 
  pivot_wider(names_from = MAG, values_from = value, values_fill = NA)
# head(classify_keggs)
```
...
# pdf("../si-figures/SI-SAR-Dictyocho-PCA-byMAG.pdf", h = 8, w = 12)
pca_shared_all %>% 
...
```{r Isolate mapped metaT info associated with vita-selected genes}
subset_mapped_transcripts <- metaT_mapped_trophy %>% 
  type.convert(as.is = TRUE) %>% 
  pivot_longer(cols = starts_with("ERR"), names_to = "run_accession", values_to = "transcript_tpm") %>%
  # Remove zero counts
  filter(transcript_tpm > 0) %>% 
  # Remove Kegg IDs with no information
  filter(KO != "NoKID") %>%
  # Add annotation information
  left_join(kegg_key_dictyo %>% select(KO, Gene_identification, VITA, B, C) %>% distinct()) %>% 
  # Remove genes with no protein ID
  filter(!is.na(Gene_identification)) %>%
  # Select only VITA chosen KEGG IDs
  filter(VITA == "vita") %>% 
  # Join with metadata
  left_join(metaT_reformat) %>%
  unite(col = "Annotation", KO, Gene_identification, sep = " ", remove = FALSE) %>%
  # summarise:
  group_by(MAG, Annotation, KO, PredictedTrophicMode, HeterotrophyScore, Gene_identification, B, C) %>% 
    summarise(SUM = sum(transcript_tpm)) %>% 
  data.frame
# head(subset_mapped_transcripts)
# 554 different annotations

# option to save dataframe
# save(subset_mapped_transcripts, photo_excl, photo_core, hetero_excl, hetero_core, shared_allmags, across_trophy, file = "../data/MAG-casestudy-Stramenopile-mappedmetaT-categories.RData")
```

Of those subset genes, how many belong to the various categories?
```{r Function to isolate list of genes by category}
generate_list_keggs <- function(df, query){
  query_list <- as.character(unique(query$KO))
  df_out <- df %>% 
    filter(KO %in% query_list)
  print(length(unique(df_out$Annotation)))
  return(df_out)
}
photo_E <- generate_list_keggs(subset_mapped_transcripts, photo_excl) #123
photo_C <- generate_list_keggs(subset_mapped_transcripts, photo_core) #29
het_E <- generate_list_keggs(subset_mapped_transcripts, hetero_excl) #90
het_C <- generate_list_keggs(subset_mapped_transcripts, hetero_core) #27
shared_ALL <- generate_list_keggs(subset_mapped_transcripts, shared_allmags) #9
across_TROPHY <- generate_list_keggs(subset_mapped_transcripts, across_trophy) #341
```
### Explore relative metaT TPM by MAG and heterotrophy score

Plot comparisons
```{r Isolate kegg categories and plot function, fig.height=30, fig.width=15}
# head(photo_E)
# Factor MAG list so it is in order of heterotrophy score

plot_bymag <- function(df, occur){
# Make tmp dataframe and factor MAG x-axis by heterotrophy score
mag_list_tmp <- df %>% 
  select(MAG, HeterotrophyScore) %>% distinct()
mag_ordered <- (mag_list_tmp$MAG)[order(mag_list_tmp$HeterotrophyScore)]

# make a new df with occurence information
df_revised <- df %>% 
  # Group by annotation and MAG, remove rows if there is an NA and it is duplicated
  group_by(Annotation, MAG) %>% 
  filter_at(vars(Annotation), any_vars(!duplicated(.)|is.na(.))) %>% 
  ungroup() %>% 
  group_by(Annotation) %>% 
  mutate(OCCUR = n_distinct(MAG)) %>% 
  filter(OCCUR >= occur)
# make tmp dataframe and factor annotation y-axis by occurence
annot_list_tmp <- df_revised %>% 
  select(Annotation, OCCUR) %>% distinct()
annot_order <- (annot_list_tmp$Annotation)[order(annot_list_tmp$OCCUR)]

# Factor revised dataframe
df_revised$MAG_order <- factor(df_revised$MAG, levels = rev(mag_ordered))
df_revised$ANNOT_order <- factor(df_revised$Annotation, levels = rev(annot_order))

ggplot(df_revised, aes(x = MAG_order, y = ANNOT_order, size = log(SUM), fill = HeterotrophyScore)) +
  geom_point(shape = 21, aes(fill=HeterotrophyScore)) +
  scale_fill_gradient2(low = "#B4CC3D",
                       mid = "white",
                       high = "#CC3D6D",
                       midpoint = 0,
  ...
  theme_half_open() +
  facet_grid(C~PredictedTrophicMode, space = "free", scales = "free") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 10),
        axis.text.y = element_text(size = 10),
        strip.background = element_blank(),
        strip.text.y = element_text(angle = 0, size = 7, hjust = 0, vjust = 0.5),
        plot.title = element_text(size = 8)) +
  labs(title = "Transcripts expressed by MAG\nSelected KEGGs = vita-selected & MAG distribution")
}
```

```{r, fig.height=16, fig.width=12}
# svg("photo_excl_bubble.svg", w = 12, h = 16)
plot_bymag(photo_E, 6)
# dev.off()
```

```{r, fig.height=10, fig.width=14}
# svg("photo_core_bubble.svg", w = 14, h = 10)
plot_bymag(photo_C, 6)
# dev.off()
```
```{r, fig.height=8, fig.width=15}
# svg("het_core_bubble.svg", w = 15, h = 8)
plot_bymag(het_C, 6)
# dev.off()
```

```{r, fig.height=15, fig.width=13}
# svg("het_excl_bubble.svg", w = 13, h = 15)
plot_bymag(het_E, 6)
# dev.off()
```

```{r, fig.height=35, fig.width=18}
# svg("across_bubble.svg", w = 18, h = 40)
# plot_bymag(across_TROPHY, 11)
# dev.off()
```

```{r, fig.height=6, fig.width=14}
# svg("shared-ALL_bubble.svg", w = 14, h = 6)
# plot_bymag(shared_ALL, 5)
# dev.off()
```

## Subset vita-selected genes - heatmap by sample

An untargetted approach up to this point has distilled down the list of genes. Continue exploring differences in the metatranscriptome expression profile across MAGs.

Option to load from previous data.
```{r Option to load from previous Rdata}
# load("../data/MAG-casestudy-Stramenopile-mappedmetaT-categories.RData", verbose = TRUE)
```

Select the vita-associated IDs.
```{r Subset vita-selected IDs}
head(subset_mapped_transcripts)
# length(unique(subset_mapped_transcripts$Annotation))
vita_only <- subset_mapped_transcripts %>% 
  select(MAG, KO, SUM) %>% distinct() %>% 
  pivot_wider(id_cols = MAG, names_from = KO,
              values_from = SUM, values_fill = 0) %>% 
  column_to_rownames(var = "MAG")
```

```{r Extrac annotation for pheatmap}
# Annotation
annot_mag <- subset_mapped_transcripts %>% 
  select(MAG, PredictedTrophicMode) %>% distinct() %>% 
  column_to_rownames(var = "MAG")

rownames(annot_mag) <- rownames(vita_only)
unique(annot_mag$PredictedTrophicMode)

annot_color = list(PredictedTrophicMode = c(Phototroph = "#B4CC3D", Heterotroph = "#CC3D6D"))
```

```{r pheatmap with all vita-selected, fig.height=6, fig.width=16}
pheatmap(vita_only, 
         annotation_row = annot_mag,
...
## Isolate specific gene sets
Option to load from previous data.
```{r Repeat load of data}
# load("../data/MAG-casestudy-Stramenopile-mappedmetaT-categories.RData", verbose = TRUE)
# subset_mapped_transcripts == vita selected only kegg IDs
```

Filter by occurrence and size.
```{r Filter by occurrence and size of metaT mapped}
# head(photo_C); head(het_C)
photo_c_list <- as.character(unique(photo_core$KO));length(photo_c_list)
het_c_list <- as.character(unique(hetero_core$KO));length(het_c_list)
shared <- as.character(unique(shared_allmags$KO));length(shared)

# head(subset_mapped_transcripts)

subset_vita_target <- subset_mapped_transcripts %>% 
  select(MAG, Annotation, KO, PredictedTrophicMode, HeterotrophyScore, SUM) %>% 
  filter(KO %in% photo_c_list | KO %in% het_c_list | KO %in% shared) %>% 
  # Group by annotation and MAG, remove rows if there is an NA and it is duplicated
  group_by(Annotation, MAG) %>% 
  filter_at(vars(Annotation), any_vars(!duplicated(.)|is.na(.))) %>% 
  ungroup() %>% 
  group_by(Annotation) %>% 
  mutate(OCCUR = n_distinct(MAG)) %>% 
  filter(OCCUR >= 11 & OCCUR < 22) # between 50-75%

length(unique(subset_vita_target$KO))
# View(subset_vita_target)

# Target gene set:
kegg_set <- as.character(unique(subset_vita_target$KO))
```


Pheatmap with target gene set:
```{r set up data frame for pheatmap with subset of genes}
# length(unique(subset_mapped_transcripts$Annotation))
targetgene_26_byMAG <- subset_mapped_transcripts %>% 
  filter(KO %in% kegg_set) %>% 
  select(MAG, Annotation, SUM) %>% distinct() %>% 
  pivot_wider(id_cols = MAG, names_from = Annotation,
              values_from = SUM, values_fill = 0) %>% 
  column_to_rownames(var = "MAG")
# dim(targetgene_26_byMAG)
```

```{r Extract annotation information by trophic mode}
# Annotation
annot_mag <- subset_mapped_transcripts %>% 
  select(MAG, PredictedTrophicMode) %>% distinct() %>% 
  column_to_rownames(var = "MAG")
...
```{r Pheatmap by trophic mode and vita-selected genes, fig.height=15, fig.width=16}
# svg("heatmap-26-KOs.svg", h = 15, w = 16)
pheatmap(targetgene_26_byMAG, 
         annotation_row = annot_mag,
         annotation_colors = annot_color,
         annotation_names_row = FALSE,
         scale = "column",
...
Generate p/a table
```{r Table for subset of genes}
p_a_table <- subset_mapped_transcripts %>% 
  filter(KO %in% kegg_set) %>% 
  select(MAG, KO, SUM) %>% distinct() %>% 
  ...
  pivot_wider(id_cols = MAG, names_from = KO,
              values_from = bin, values_fill = -1)
# dim(p_a_table)
# write_delim(p_a_table, path = "subset-keggIDs-stramenopileMAGs-27-04-2021.txt", delim = "\t")

gene_set_list <- subset_mapped_transcripts %>% 
  filter(KO %in% kegg_set) %>% select(Annotation, KO) %>% distinct()

# View(gene_set_list)
```

## Biogeography of hypothesized MAST & Dictyochophyceae MAGs
...
Relative abundance of each MAG by region and depth
```{r}
# Bubble plot x3 for each MAG, where region = y and depth = x axis. Try with size frac to confirm it isn't as important?
```


Visualize population-level differential gene expression
```{r}
# lollipop plot based on the 27 core gene sets of interest and 3 selected MAGs (based on hylogeny and relativea bundance.)
# COlor = ocean region for all 27 genes (x-axis), yaxis == lollipop value for transofmred metaT TPM value. 
```


## Exploration of visuals to look at expression of this gene set
Generate t-SNE plots for SI material.
```{r Starting data types}
# head(metaT_mapped_trophy)
vita_keggs_list <- as.character(unique(kegg_vita$Selected.KOs))
# length(unique(metaT_mapped_trophy$MAG))

# Prepare metaT data for t-SNE input (of the 24 MAGs)
all_data_24 <- metaT_mapped_trophy %>% 
  # Select vita KEGG ids only
  filter(KO %in% vita_keggs_list) %>% 
  pivot_longer(cols = starts_with("ERR"), names_to = "run_accession", values_to = "tpm") %>%
  left_join(metaT_reformat)

dim(all_data_24)
unique(all_data_24$HeterotrophyScore)
# Prepare numeric data
# SAMPLES == MAG + ERR
depths <- c("SRF", "DCM")

numeric_data_SRF <- all_data_24 %>%
  filter(DEPTH %in% depths) %>% 
  unite(SAMPLE, c("MAG", "run_accession"), sep = "-") %>%
  select(SAMPLE, KO, tpm) %>% distinct() %>%
  group_by(SAMPLE, KO) %>%
    summarise(sum_tpm = sum(tpm)) %>%
  pivot_wider(id_cols = SAMPLE, names_from = KO,
              values_from = sum_tpm, values_fill = 0) %>%
  column_to_rownames(var = "SAMPLE")
dim(numeric_data_SRF)
# 
meta_data_SRF <- all_data_24 %>%
  filter(DEPTH %in% depths) %>% 
  unite(SAMPLE, c("MAG", "run_accession"), sep = "-") %>%
  select(SAMPLE, REGION, DEPTH, SIZEFRAC, PredictedTrophicMode, HeterotrophyScore) %>% distinct() %>% 
  column_to_rownames(var = "SAMPLE")
# head(meta_data_SRF)
```
...
```{r Run at 100 perplexity}
# tsne_srf_10 <- Rtsne(numeric_data_SRF, perplexity=10, check_duplicates = FALSE)
# tsne_srf_50 <- Rtsne(numeric_data_SRF, perplexity=50, check_duplicates = FALSE)
tsne_srf_100 <- Rtsne(numeric_data_SRF, perplexity=100, check_duplicates = FALSE)
```

```{r Extract tSNE info and factor}
# Extract TSNE information
tsne_df_toplot <- data.frame(x = tsne_srf_100$Y[,1], y = tsne_srf_100$Y[,2], meta_data_SRF) %>% 
  rownames_to_column(var = "SAMPLE") %>% 
  separate(SAMPLE, c("MAG", "ERR"), sep = "-", remove = FALSE)
...
tsne_df_toplot$FRAC_ORDER <- factor(tsne_df_toplot$SIZEFRAC, levels = frac_order)
tsne_df_toplot$DEPTH_ORDER <- factor(tsne_df_toplot$DEPTH, depth_order)
tsne_df_toplot$REGION_ORDER <- factor(tsne_df_toplot$REGION, region_order)
names(frac_color) <- frac_order
names(depth_color) <- depth_order
names(region_color) <- region_order

tsne_df_toplot$TROPHY_ORDER <- factor(tsne_df_toplot$PredictedTrophicMode, levels = c("Heterotroph", "Phototroph"))
trophy_color <- c("#CC3D6D", "#B4CC3D")
names(trophy_color) <- c("Heterotroph", "Phototroph")
```

```{r 4 panel tSNE plot, fig.height=9, fig.width=12}
# pdf("SI-topography-t-SNE-StramenopileMAGs.pdf", h=9, w=12)
plot_grid(ggplot(tsne_df_toplot, aes(x = x, y = y, color = REGION_ORDER)) + 
  geom_point(shape = 19, size = 2, aes(color = REGION_ORDER)) +
  scale_color_manual(values = region_color) +
  theme_linedraw() +
  theme(axis.text = element_blank(),
        legend.title = element_blank()),
ggplot(tsne_df_toplot, aes(x = x, y = y, color = FRAC_ORDER)) + 
  geom_point(shape = 19, size = 2, aes(color = FRAC_ORDER)) +
  scale_color_manual(values = frac_color) +
  theme_linedraw() +
  theme(axis.text = element_blank(),
        legend.title = element_blank()),
ggplot(tsne_df_toplot, aes(x = x, y = y, color = DEPTH_ORDER)) + 
  geom_point(shape = 19, size = 2, aes(color = DEPTH_ORDER)) +
  scale_color_manual(values = depth_color) +
  theme_linedraw() +
  theme(axis.text = element_blank(),
        legend.title = element_blank()),
ggplot(tsne_df_toplot, aes(x = x, y = y, color = TROPHY_ORDER)) + 
  geom_point(shape = 19, size = 2, aes(color = TROPHY_ORDER)) +
  scale_color_manual(values = trophy_color) +
  theme_linedraw() +
  theme(axis.text = element_blank(),
        legend.title = element_blank()),
