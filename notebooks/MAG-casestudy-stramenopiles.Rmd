...
par(mfrow=c(2,2))
# plot(tsne_srf$Y, col = "blue", pch = 19, cex = 0.5)
# xy <- as.data.frame(tsne_srf$Y)
# head(xy)
plot(tsne_srf_100$Y, pch = 21, cex = 1, bg= as.factor(meta_data_SRF$SIZEFRAC), col = "black")
plot(tsne_srf_50$Y, pch = 21, cex = 1, bg= as.factor(meta_data_SRF$SIZEFRAC), col = "black")
plot(tsne_srf_100$Y, pch = 21, cex = 1, bg= as.factor(meta_data_SRF$PredictedTrophicMode), col = "black")
plot(tsne_srf_50$Y, pch = 21, cex = 1, bg= as.factor(meta_data_SRF$PredictedTrophicMode), col = "black")
...
# ggplot(xy, aes(x = V1, V2))
```
```{r}
# Extract TSNE information
tsne_df_toplot <- data.frame(x = tsne_srf$Y[,1], y = tsne_srf$Y[,2], meta_data_SRF) %>% 
  rownames_to_column(var = "SAMPLE") %>% 
  separate(SAMPLE, c("MAG", "ERR"), sep = "-", remove = FALSE)

head(tsne_df_toplot)

tsne_plot_fxn <- function(tsne_df, fill_var){
  ggplot(tsne_df, aes(x = x, y = y, fill = fill_var)) + 
    geom_point(shape = 21, aes(fill = fill_var)) +
    theme_linedraw()
}
```

```{r, fig.height=10, fig.width=10}
# tsne_plot_fxn(tsne_df_toplot, REGION)
ggplot(tsne_df_toplot, aes(x = x, y = y, fill = REGION)) + 
...
```{r}
# numeric_data <- subset_mapped_transcripts %>% 
#   select(MAG, Annotation, KO, PredictedTrophicMode, HeterotrophyScore, SUM) %>% 
#   # Group by annotation and MAG, remove rows if there is an NA and it is duplicated
#   group_by(Annotation, MAG) %>% 
#   filter_at(vars(Annotation), any_vars(!duplicated(.)|is.na(.))) %>%
...
# 
# head(subset_mapped_transcripts)
# subset_mapped_transcripts %>% 
#   select(MAG, PredictedTrophicMode, HeterotrophyScore)
# Numeric data: rows are samples, and columns are genes
# metadata, rownames are the same as samples
```
